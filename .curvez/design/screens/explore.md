...
goal: 카테고리와 서울 구를 바꿔가며 벽을 훑고 마음에 드는 것을 담는다
...
- state:error — wall 만 ErrorPanel 로 치환. (en) "We couldn't load this wall." / (ko) "이 벽을 불러오지 못했습니다." 재시도 버튼 1개. filter 선택값은 유지한다
...
- announce: 필터 변경 시 live region 으로 "<카테고리>, <구> 벽으로 바뀌었습니다". 건수를 읽지 않는다
