...
goal: 서울을 편집된 목록으로 처음 보여주고, 방문자가 최소 하나를 담게 만든다
...
- region: masthead
  ...
  - note: sticky 로 만들지 않는다. 스크롤하면 목록만 남는다
- region: opening
...
- state:empty — TourAPI 가 0건을 반환한 경우다. 문구(en) "Nothing on the wall right now." / (ko) "지금은 목록이 비어 있습니다." 아래에 "다시 불러오기" 버튼 1개. 카테고리 필터가 걸린 wall-category 에서는 문구를 "이 카테고리에는 아직 걸어둔 것이 없습니다" 로 나눈다
- state:error — wall 영역만 ErrorPanel 로 치환한다. 문구(en) "We couldn't load the wall." / (ko) "목록을 불러오지 못했습니다." 재시도 버튼 1개. masthead·opening·footer 는 유지한다. 일부만 실패하면 성공한 액자는 그대로 두고 실패한 슬롯만 치환한다
...
- announce: 카테고리 전환 시 live region 으로 "<카테고리> 목록으로 바뀌었습니다"
