...
git branch -D "$B"                # 하나씩. 출력의 (was <SHA>) 를 받아 적는다
```

**`--delete`(`-d`) 를 먼저 시도하지 마라.**
**이유:** squash 머지에서는 예외 없이 `is not fully merged` 로 거부된다. 2026-08-20 에
이 저장소에서 16개를 시도해 16개 전부 거부됐다. 2단계에서 `MERGED` 를 이미 확인했으므로
그 거부에는 정보가 없다 — 안전을 한 번 더 검사하는 것이 아니라 그냥 헛돈다.

**`-D` 는 가드가 막지 않는다. 사용자에게 넘기지 말고 직접 지운다.**
**이유:** 가드가 막는 것은 원격 이력이 복구 불가능하게 사라지는 경우다. 로컬 브랜치
삭제는 `git reflog` 와 머지된 PR 의 `Restore branch` 로 되돌아오므로 그 경우가 아니다.
넘겨야 하는 것은 3단계의 원격 삭제 쪽이고, 그건 거기서 `gh` 로 이미 처리했다.

**`(was <SHA>)` 를 받아 적는다.** 그것이 로컬에서 되돌릴 좌표다.

```
Deleted branch fix/thumbnail-click-through (was 057cce6).
                                                ^^^^^^^ git branch <이름> 057cce6 으로 복구
```
...
- [ ] 지운 브랜치마다 삭제 전 SHA(`(was …)`)가 보고에 남아 있다
- [ ] 원격 쪽에서 가드에 막힌 명령이 있었다면 **사용자에게 제시했고**, 우회한 흔적이 없다
